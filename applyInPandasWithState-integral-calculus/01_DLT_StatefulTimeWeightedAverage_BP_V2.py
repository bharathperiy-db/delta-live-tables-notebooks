...
def pd_time_weighted_average(location_id, sensor, timestamp_window, values_arr, timestamps_arr):
    ...
    Returns:
    --------
    time_interval_weighted_avg(float): Time-weighted average value for the window.
    count_rows(int): Number of rows in the window.
    """
    ...
    # Return the TimeWeighted Avg & Count of Records
...
# actual_time_interval_weighted_avg, actual_count_rows = pd_time_weighted_average(
#   location_id="L1", 
#   sensor="Wind_Speed", 
...
# expected_time_interval_weighted_avg = 29.0
# expected_count_rows = 4

# # Check results
# assert actual_time_interval_weighted_avg == expected_time_interval_weighted_avg, f"Expected time_interval_weighted_avg to be {expected_time_interval_weighted_avg} but got {actual_time_interval_weighted_avg}"
# assert actual_count_rows == expected_count_rows, f"Expected count_rows to be {expected_count_rows} but got {actual_count_rows}"
...
# MAGIC
# MAGIC The important concept here is this function is called for each key group that arrives in the input stream; in other words, for each tuple of `(location_id, sensor, timestamp_window)`, this function runs one time.
...
def stateful_time_weighted_average(key, df_pds: Iterator[pd.DataFrame], state: GroupState) -> pd.DataFrame:
  ...
  time_weighted_row: Pandas Row 
    Row object with schema: location_id string, sensor string, timestamp_window {start: timestamp, end: timestamp}, value float, count_rows int
  """

  # Read Grouping Keys
  (location_id, sensor, timestamp_window) = key

  # Calculate the Time interval for group, in minutes
  interval_mins = (timestamp_window['end'] - timestamp_window['start']).seconds / 60
  ...
  if state.hasTimedOut:
    ...
    # Return a dataframe row to the calling stream
...
def dlt_integrals():
  ...
  # Define the Watermark (late-arrival threshold)
  watermark_duration = '10 minutes'

  # Time Window for Calculating time-weighted avg, windows will be of fixed interval (10:00 - 10:10, 10:10 - 10:20, ..)
  window_duration = '10 minutes'

  # Apply watermark to our stream based on time interval, then groupBy.Apply() logic
  grp = (df
        # Use window() function to determine thhe interval group each record belongs to
        .withColumn('timestamp_window', window('timestamp', window_duration))
        # Configure the watermark (late-arrival) for the stream
        .withWatermark('timestamp', watermark_duration)
        # Group records by location, sensor, timestamp_window
