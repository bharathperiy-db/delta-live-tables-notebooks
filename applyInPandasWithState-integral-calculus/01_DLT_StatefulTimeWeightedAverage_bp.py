...
from pyspark.sql.types import StructType, StructField, ArrayType, TimestampType, FloatType, IntegerType, StringType, MapType
...
def stateful_time_weighted_average(key, df_pds: Iterator[pd.DataFrame], state: GroupState) -> pd.DataFrame:
  ...
  if state.hasTimedOut:
    ...
    time_weighted_row = pd_time_weighted_average(location_id, sensor, timestamp_window, values_arr, timestamps_arr)
...
def dlt_integrals():
  ...
  # Define the Watermark Time
  watermark_duration = '10 minutes'
  
  # Grouping Window time
  window_duration = '10 minutes'

  # Apply watermark to our stream based on time interval, then groupBy.Apply() logic
  grp = (df
        .withColumn('time_window', window('timestamp', window_duration)) # Define the interval for the weighthed avg
        .withWatermark('timestamp', watermark_duration) # Define the watermark for state
        .groupBy('location_id', 'sensor', 'time_window')
